...
class RewardStrategy(object):
    ...
    def determine_multiplier(self, env, action):
        ...
        if (
            unique_seq
            and (
                self.door_open(env, action)
                or self.door_unlocked_partial_action_seq(env)
            )
        ...
        if (
            index != -1
            and num_solutions_found != 0
            and index != first_solution_index
            and (
                self.door_open(env, action)
                or self.door_unlocked_partial_action_seq(env)
            )
    ...
    def determine_reward(self, env, action, reward_mode):
        ...
        if reward_mode == "basic":
            ...
            reward = self.reward_negative_immovable_partial_seq_solution_multiplier(
                env, action
            )
        elif (
            reward_mode
            ...
            reward = self.reward_negative_immovable_partial_seq_solution_multiplier_door_seq(
                env, action
            )
        else:
            raise ValueError(
    ...
    def reward_negative_immovable_partial_seq_solution_multiplier_door_seq(
        self, env, action
    ):
        """
        Give reward of REWARD_OPEN for opening the door with a new action sequence

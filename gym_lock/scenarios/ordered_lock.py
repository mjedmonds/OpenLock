...
class MultiDoorLockFSM(object):
    ...
    def __init__(self):
        ...
        self.machine.add_transition('open', MultiDoorLockFSM.ordered_path[-2], MultiDoorLockFSM.ordered_path[-1])
        self.machine.add_transition('close', MultiDoorLockFSM.ordered_path[-1], MultiDoorLockFSM.ordered_path[-2])

        # add ordered path transitions
        for i in range(0, len(MultiDoorLockFSM.ordered_path) - 2):
            self.machine.add_transition('unlock_{}'.format(MultiDoorLockFSM.locks[i + 1]),
                                        MultiDoorLockFSM.ordered_path[i], MultiDoorLockFSM.ordered_path[i + 1])
            self.machine.add_transition('lock_{}'.format(MultiDoorLockFSM.locks[i + 1]),
                                        MultiDoorLockFSM.ordered_path[i + 1], MultiDoorLockFSM.ordered_path[i])

        # add transitions off of ordered path
        self.machine.add_transition('unlock_l3', MultiDoorLockFSM.ordered_path[0], 'l1-l2+l3-o-')
...
class MultiDoorLockMDP(StateMachineMDP):
    ...
    def __init__(self):
        ...
        for lock in MultiDoorLockFSM.locks:
            ...
            unlocked = [s for s in self.fsm.states if lock + '-' in s and not '_ord' in s]
            for locked_state, unlocked_state in zip(locked, unlocked):
                ...
                self.add_transition('lock_{}'.format(lock), unlocked_state, locked_state, 1)
                self.add_transition('unlock_{}'.format(lock), locked_state, unlocked_state, 1)
        ...
        # add door open/close transition
        self.add_transition('open', MultiDoorLockFSM.ordered_path[-2], MultiDoorLockFSM.ordered_path[-1], 1)
        self.add_transition('close', MultiDoorLockFSM.ordered_path[-1], MultiDoorLockFSM.ordered_path[-2], 1)

        # add ordered path transitions
        for i in range(0, len(MultiDoorLockFSM.ordered_path) - 2):
            self.add_transition('unlock_{}'.format(MultiDoorLockFSM.locks[i + 1]),
                                        MultiDoorLockFSM.ordered_path[i], MultiDoorLockFSM.ordered_path[i + 1], 1)
            self.add_transition('lock_{}'.format(MultiDoorLockFSM.locks[i + 1]),
                                        MultiDoorLockFSM.ordered_path[i + 1], MultiDoorLockFSM.ordered_path[i], 1)

        # add transitions off of ordered path
        self.add_transition('unlock_l3', MultiDoorLockFSM.ordered_path[0], 'l1-l2+l3-o-', 1)

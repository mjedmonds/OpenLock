...
class StateMachineMDP(object):
    ...
    def __init__(self):
        self.transitions = np.zeros((len(self.fsm.actions), len(self.fsm.states), len(self.fsm.states)))  # A x S x S
    ...
    def add_transition(self, action, source, dest, prob):
        self.transitions[self.action_map[action]][self.state_map[source]][self.state_map[dest]] = prob
